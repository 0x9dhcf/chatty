...
SessionInfo SessionManager::create(const std::string &name,
                                    const ChattyConfig &cfg) {
  ...
  sqlite3_prepare_v2(
      db_, "SELECT created_at, updated_at FROM sessions WHERE uuid = ?", -1, &q,
      nullptr);
...
std::shared_ptr<agt::Session>
SessionManager::open(const std::string &uuid) {
...
void SessionManager::remove(const std::string &uuid) {
  sqlite3_stmt *stmt = nullptr;
  sqlite3_prepare_v2(db_, "DELETE FROM sessions WHERE uuid = ?", -1, &stmt,
                     nullptr);
  sqlite3_bind_text(stmt, 1, uuid.c_str(), -1, SQLITE_TRANSIENT);
  sqlite3_step(stmt);
  sqlite3_finalize(stmt);
...
void SessionManager::rename(const std::string &uuid,
                            const std::string &name) {
  sqlite3_stmt *stmt = nullptr;
  sqlite3_prepare_v2(db_, "UPDATE sessions SET name = ? WHERE uuid = ?", -1,
                     &stmt, nullptr);
...
void SessionManager::update_config(const std::string &uuid,
                                    const ChattyConfig &cfg) {
  auto provider = std::string(agt::provider_to_string(cfg.provider));
  sqlite3_stmt *stmt = nullptr;
  sqlite3_prepare_v2(
