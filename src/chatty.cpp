...
void Chatty::build_instructions() {
  ...
  p << "You are Chatty a helpful terminal assistant. Keep responses concise and\n"
    ...
    << "- For genuine tool errors (timeouts, missing files, malformed\n"
    << "  output), investigate before giving up — consider what else could\n"
    << "  explain the result and try a different approach.\n"
    << "- A tool result of exactly {\"error\": \"tool call denied\"} means\n"
    << "  the user refused. Do not retry, vary args, or reach for a near-\n"
    << "  equivalent tool. Continue without that capability or explain\n"
    << "  plainly what you needed and why.\n"
